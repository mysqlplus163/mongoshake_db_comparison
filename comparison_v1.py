...
def check(src, dst):
	...
	if configure[INCLUDE_COLLS] !=['']:
	    srcColls = [coll for coll in srcColls if coll in configure[INCLUDE_COLLS]]
	    dstColls = [coll for coll in dstColls if coll in configure[INCLUDE_COLLS]]
	else:
	...
	log_info("dstColls %s " % (dstColls))
	    ...
	    elif configure[INCLUDE_COLLS] != ['']:
		srcColl_myCount=srcColl.aggregate( [{ '$count': "myCount" }]).next()["myCount"]
		dstColl_myCount=dstColl.aggregate( [{ '$count': "myCount" }]).next()["myCount"]
		if srcColl_myCount != dstColl_myCount:
                    log_error("DIFF => collection [%s] record count not equals" % (coll))
                    if srcColl_myCount < dstColl_myCount:
                        log_error("srcColl.count: %d < dstColl.count: %d " % (srcColl_myCount,dstColl_myCount))
                    else:
                        log_error("srcColl.count: %d > dstColl.count: %d " % (srcColl_myCount,dstColl_myCount))
                else:
                    log_info("EQUL => collection [%s] record count equals" % (coll))
	    else:
		srcColl_doc_cnt=srcColl.estimated_document_count()
		dstColl_doc_cnt=dstColl.estimated_document_count()
                if srcColl_doc_cnt != dstColl_doc_cnt:
	            log_error("DIFF => collection [%s] record count not equals" % (coll))
		    if srcColl_doc_cnt < dstColl_doc_cnt:
	                log_error("srcColl.count: %d < dstColl.count: %d " % (srcColl_doc_cnt,dstColl_doc_cnt))
	            else:
			log_error("srcColl.count: %d > dstColl.count: %d " % (srcColl_doc_cnt,dstColl_doc_cnt))
                else:
                    log_info("EQUL => collection [%s] record count equals" % (coll))
...
def usage():
    print '|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|'
    print "| Usage: ./comparison.py --src=localhost:27017/db? --dest=localhost:27018/db? --count=10000 (the sample number) --excludeDbs=admin,local --excludeCollections=system.profile --includeDbs=db --includeCollections=colles --comparisonMode=sample/all/no (sample: comparison sample number, default; all: comparison all data; no: only comparison outline without data)  |"
    print '|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|'
    print '| Like : ./comparison.py --src="localhost:3001" --dest=localhost:3100  --count=1000  --excludeDbs=admin,local,mongoshake --excludeCollections=system.profile  --includeDbs=db --includeCollections=colles --comparisonMode=sample  |'
    print '|------------------------------------------------------------------------------------------------------------------------------------------------------------------------|'
    exit(0)

if __name__ == "__main__":
    opts, args = getopt.getopt(sys.argv[1:], "hs:d:n:e:x:i:c", ["help", "src=", "dest=", "count=", "excludeDbs=", "excludeCollections=", "includeDbs=", "includeCollections=", "comparisonMode="])
    ...
    for key, value in opts:
        ...
        if key in ("-c", "--includeCollections"):
    ...
    # dump configuration
    log_info("Configuration [sample=%s, count=%d, excludeDbs=%s, excludeColls=%s],includeDbs=%s, includeColls=%s]" % (configure[SAMPLE], configure[COMPARISION_COUNT], configure[EXCLUDE_DBS], configure[EXCLUDE_COLLS],configure[INCLUDE_DBS], configure[INCLUDE_COLLS]))
